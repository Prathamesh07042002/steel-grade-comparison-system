  ...
  return (
    <div className="app">
      <header className="header">
        ...
        <p>Manual and automatic comparison of mill certificates against steel standards.</p>
      </header>
...
function Pipeline1() {
  ...
  const handleStandardSelect = async (stdName) => {
    ...
    try {
      ...
      setError('Failed to load standard details: ' + (err.message || 'Unknown error'));
  ...
  return (
    <div className="pipeline">
      ...
      <p>Upload a test / mill-certificate PDF → choose the standard to compare → get a property-by-property pass / fail report</p>
      ...
      <section className="step">
        ...
        {pdfPreview && (
          <>
            <button onClick={() => setShowPdfPreview(!showPdfPreview)} className="btn-secondary">
          ...
          <div className="properties-grid">
            <div className="property-column">
              ...
              {Object.entries(testData.chemical_properties || {}).length > 0 ? (
                Object.entries(testData.chemical_properties).map(([key, val]) => (
            ...
            <div className="property-column">
              ...
              {Object.entries(testData.mechanical_properties || {}).length > 0 ? (
                Object.entries(testData.mechanical_properties).map(([key, val]) => (
                  <div key={key} className="property-item">
                    <strong>{key}:</strong> <code>{val}</code>
                  </div>
                ))
...
function Pipeline2() {
  ...
  return (
    <div className="pipeline">
      ...
      <p>Upload a test PDF → the LLM scans ALL stored standards and selects the best-matching grade automatically</p>
      ...
      {uploadedFile && (
        <section className="step">
          ...
          <p>The LLM will compare this PDF against all standard JSONs and return the best match.</p>
          <button 
...
function ComparisonResults({ chem_result, mech_result, testFilename, standardName }) {
  const allMatched = [
  ...
  return (
    <section className="results">
      ...
      <p className="result-subtitle">
        <strong>Test:</strong> {testFilename} | <strong>Standard:</strong> {standardName}
      </p>
      ...
      <div className={`verdict-card ${verdictClass}`}>
        ...
        <div className="verdict-details">
          ...
          <div className="progress-bar">
            <div className="progress-fill" style={{ width: `${passPercent}%` }}></div>
          </div>
          <p className="match-summary">{passCount} of {total} properties passed</p>
        </div>
      </div>
            ...
            <tbody>
              {Object.entries(matched).map(([prop, info]) => (
                <tr key={prop} className={info.within_range ? 'pass' : 'fail'}>
                  ...
                  <td><code>{info.test_value}</code></td>
                  <td><code>{info.standard_value}</code></td>
                  <td className={info.within_range ? 'pass' : 'fail'}>
                    {info.within_range ? '✅ PASS' : '❌ FAIL'}
                  </td>
                  <td><em>{info.note || '-'}</em></td>
                </tr>
              ))}
            </tbody>
          </table>
              ...
              <table className="info-table">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>{(result.result.closest_attempt.overall_score * 100).toFixed(1)}%</td>
                  </tr>
                  <tr>
  ...
  return (
    <section className="results auto-match">
      <div className={`verdict-card ${best.verdict.toLowerCase()}`}>
        <h3>{best.verdict === 'MATCHED' ? '✅' : '⚠️'} {best.verdict}</h3>
        <div className="verdict-details">
          <p className="match-score">{passPercent}% Match Score</p>
          <div className="progress-bar">
            <div className="progress-fill" style={{ width: `${passPercent}%` }}></div>
          </div>
          <p className="match-summary">{passCount} of {totalCount} properties matched</p>
        </div>
      </div>
      ...
      <div className="best-match-info">
        ...
        <table className="info-table">
          <tbody>
            ...
            <tr>
              <td><strong>Standard File:</strong></td>
            ...
            <tr>
              ...
              <td>{best.name_match ? '✅ Yes' : '❌ No'} {best.name_match_reason}</td>
            </tr>
            <tr>
              <td><strong>Verdict Reason:</strong></td>
            ...
            <tbody>
              {topMatches.map((match, idx) => (
                <tr key={idx}>
                  ...
                  <td><strong>{(match.overall_score * 100).toFixed(1)}%</strong></td>
                  <td>{match.matched_count}/{match.standard_total}</td>
                  <td>{match.verdict}</td>
                </tr>
              ))}
            </tbody>
...
function parseProperties(params) {
  const MECH_KEYWORDS = new Set([
    ...
    "r-bar", "n-value", "mpa", "hv", "hrc", "strength"
  ]);
  ...
  for (const p of (params || [])) {
    ...
    if (Array.from(MECH_KEYWORDS).some(kw => elem.toLowerCase().includes(kw))) {
      mech[elem] = rv;
    } else {
      chem[elem] = rv;
    }
...
function renderStandardProperties(params, type) {
  ...
  const filtered = (params || []).filter(p => {
    const elem = p.element || '';
    const isMech = Array.from(MECH_KEYWORDS).some(kw => elem.toLowerCase().includes(kw));
